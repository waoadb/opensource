...
export const ToastNotification = ({
  ...
  return (
    <div className={`p-4 border-2 mb-4 max-w-screen-sm ${toastClasses[type]} ${className}`}>
      <div className="flex w-full gap-4 relative border-inherit pr-10 items-center text-inherit">
        {icon && <span className="bg-gray-100 text-gray-500 min-w-9 h-9 inline-flex items-center justify-center rounded-full">
          {icon}
        </span>}
        <span className="block text-gray-700 leading-snug">
          <span className="block text-gray-900 text-lg">Toast title here</span>
          <span className="block text-sm">Lorem ipsum dolor sit amet, consectetur adipisicing elit.</span>
        </span>
        ...
        <button className="p-2 bg-white rounded-md border-2 border-inherit absolute text-inherit opacity-50 hover:opacity-100 right-0 top-1/2 -translate-y-1/2">
          ...
          <Icon name="Close" width="16" height="16" className="text-gray-500 hover:text-gray-700" />
        </button>
      </div>
    </div>
  );
